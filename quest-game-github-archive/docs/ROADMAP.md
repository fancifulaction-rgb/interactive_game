...
| # | Задача | ID каталога | Критерий готовности |
|---|--------|-------------|---------------------|
| 2.1 | Комната ожидания (общий старт) | IMP-UX-001 | Команды в lobby; игра не начинается до host start |
| 2.2 | Экран ведущего + QR на регистрацию | IMP-UX-002, IMP-UX-003 | `/host/:code` или расширение admin scoreboard |
| 2.3 | PWA (manifest + service worker) | IMP-UX-004 | Установка на домашний экран iOS/Android |
| 2.4 | Блокировка табло до финиша (настройка игры) | IMP-UX-005 | Флаг в `games.settings` |
| 2.5 | Код игры 6 символов + буквы | IMP-PRD-001 | Генерация и валидация |
...
| # | Задача | ID каталога | Критерий готовности |
|---|--------|-------------|---------------------|
| 3.1 | Realtime Broadcast для счёта | IMP-RT-001 | Меньше postgres_changes на teams |
| 3.2 | Убрать poll 5s на AdminScoreboard | IMP-RT-002 | Только Broadcast/один channel |
| 3.3 | Серверная проверка авто-ответов | IMP-LOG-001 | Edge или trigger; клиент не шлёт is_correct |
| 3.4 | AI-генерация вопросов (Gemini Edge) | IMP-PRD-002 | Кнопка в GameEditor, опционально |
