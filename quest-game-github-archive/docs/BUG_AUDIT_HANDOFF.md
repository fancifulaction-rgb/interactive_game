...
> ⚠️ **Актуальный предрелизный аудит — [AUDIT_PRERELEASE_2026-06-11.md](AUDIT_PRERELEASE_2026-06-11.md)** (новые находки P0/P1/P2, IMP-ID раздела M каталога, промт для агента). Этот документ (2026-06-07) — закрытый предыдущий цикл (C/H/M/S/L), оставлен как контекст «что уже исправлено».
...
## Ручной QA 2026-06-11 (организатор)

| Сценарий | Результат |
|----------|-----------|
| AI 5 текстовых вопросов + сохранение | OK |
| QR → регистрация → лобби → игра → финиш | OK |
| Скрытие вопроса (4 из 5) | OK |
| Кнопка «Копировать» ссылку регистрации | OK (fix: `copyToClipboard.ts`) |
| Фото в вопросе (редактор) | OK (fix: JWT upload + `randomUuid` на HTTP/LAN) |
| Инкогнито без `?join=` в URL | ожидаемо (SEC-019); копировать полную ссылку из поля |

**Повторная проверка:** ✅ копировать → инкогнито → регистрация; JPG/PNG в редакторе → сохранение (~367ms upsert x5).
