...
Фокус — багфиксы по `docs/BUG_AUDIT_HANDOFF.md` (раздел «Рекомендуемый порядок работ»:
C1 → H2/H3 → H1 → H7 → H4/M7 → H5 → H6 → C2/M6). Security-блок **S1–S6 — только по
согласованию** (миграции + Edge + сессия команды), не катить на прод вслепую. Сделано:
realtime-fallback (admin poll 6с; player postgres_changes + частый poll на паузе).
...
• Бери ОДИН пункт по порядку из раздела «Рекомендуемый порядок работ» аудита
  (по умолчанию: C1 → H2/H3 → H1 → H7 → H4/M7 → H5 → H6 → C2/M6 → S1–S6 (по согласованию) → S7/L1–L6).
