Deno.serve(async (req) => {
    ...
    try {
        ...
        // Create a simple test file (text content)
        const testContent = 'Test file content for upload';
        const fileName = `test-upload-${Date.now()}.txt`;

        // Test upload using both anon and service role keys
        const uploadTests = [];

        // Test 1: Upload to question-media with anon key (same as frontend)
        try {
            ...
            uploadTests.push({
                ...
                message: uploadResponse.ok ? 'File uploaded successfully' : await uploadResponse.text()
            });
        ...
        try {
            ...
            uploadTests.push({
                ...
                message: uploadResponse2.ok ? 'File uploaded successfully' : await uploadResponse2.text()
            });
        ...
        // Test 3: Upload with service role (should always work)
        try {
            const uploadResponse3 = await fetch(`${supabaseUrl}/storage/v1/object/answer-media/service-${fileName}`, {
                method: 'POST',
                headers: {
            ...
            uploadTests.push({
                test: 'Upload to answer-media (service role)',
                status: uploadResponse3.ok ? 'success' : 'failed',
                status_code: uploadResponse3.status,
                message: uploadResponse3.ok ? 'File uploaded successfully' : await uploadResponse3.text()
            });
        } catch (error) {
            uploadTests.push({
                test: 'Upload to answer-media (service role)',
        ...
        return new Response(
            JSON.stringify({ 
                success: true,
                message: `Upload test completed: ${successfulUploads}/${totalTests} tests successful`,
                ...
                recommendation: successfulUploads === totalTests ? 
                    '✅ All uploads work! You can now test file upload in your game.' :
